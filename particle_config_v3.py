...
fig = plt.figure()
ax = fig.add_subplot(111, title='Most Probable Configuration(s)', xlabel='Number of Particles', ylabel='Energy Level',)
...
result_text1 = "$\Sigma$ $W$ = {0:.0f}".format(totalWeight)
result_text2 = "$P$ = {0:.3f}".format(prob_MostProbConfig2)

fig.text(0.65, 0.8, result_text1)
fig.text(0.65, 0.7, result_text2)

plt.show()

fig.savefig("./png/distribution_M{0}_E{1}_N{2}.png".format(NumLevels,TotalEnergy,NumParticles), dpi=300)
